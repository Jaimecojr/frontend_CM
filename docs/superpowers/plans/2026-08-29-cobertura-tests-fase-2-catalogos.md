...
- [x] **Step 1: Tests de `getAgreements`**
...
- [x] **Step 2: Tests de `getAgreement`**
...
- [x] **Step 3: Tests de `getDepartments`/`getCitiesByDepartment`**
...
- [x] **Step 4: Tests de `createAgreement` y `updateAgreement`**
...
- [x] **Step 5: Tests de `updateAgreementState` — la firma real (id, agreement completo, newState)**
...
- [x] **Step 6: Correr `npm run test` y `npx tsc --noEmit`**
...
- [x] **Step 1: Test de las columnas `id`, `name`, `amount`, `city`**
...
- [x] **Step 3: Test de la columna `actions` — gate de `canView`/`canManage` independientes**
...
- [x] **Step 2: Test de visibilidad por `mode`**
...
- [x] **Step 3: Test de validación de `amount`**
...
- [x] **Step 4: Test de `canSubmit` y `submit`**
...
- [x] **Step 1: Test de `page.tsx` — gates de `canView`/`canManage` y el botón de crear**
...
- [x] **Step 2: Test de `page.tsx` — el `updateFn` con resolución por ref (caso especial)**
...
- [x] **Step 3: Tests de `new/page.tsx` — gate por `useEffect` + redirect**
...
- [x] **Step 4: Tests de `[id]/page.tsx` (vista) — SIN gate de permisos**
...
- [x] **Step 5: Tests de `[id]/edit/page.tsx`**
...
- [x] **Step 1: Tests de `getCounselors` y `getCounselor`**
...
- [x] **Step 2: Tests de `getDepartments`/`getCitiesByDepartment`/`getActiveFranchises`**
...
- [x] **Step 3: Tests de `createCounselor`, `updateCounselor`, `updateCounselorState`**
...
- [x] **Step 4: Tests de `checkCounselorIdCard`**
...
- [x] **Step 1: Test de las columnas `full_name`, `id_card`, `movil`, `city`**
...
- [x] **Step 3: Test de la columna `actions` — gate único de `hasAccess` (sin sub-gate de
  `canManage`)**
...
- [x] **Step 4: Test de `submit` — revalidación forzada de cédula antes de guardar**
...
- [x] **Step 1: Test de `page.tsx` — gate de `hasAccess` para el botón "Crear Asesor"**
...
- [x] **Step 2: Tests de `new/page.tsx` — sin gate**
...
- [x] **Step 4: Tests de `[id]/edit/page.tsx` — sin gate**
...
- [x] **Step 1: Tests de `getFranchises` — el filtro de `SuperAdmin` y la normalización de `type`**
...
- [x] **Step 2: Tests de `getFranchise`, `getDepartments`, `getCitiesByDepartment`**
...
- [x] **Step 3: Tests de `createUser`, `updateFranchise`, `updateFranchiseState`**
